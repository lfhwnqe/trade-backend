...
import { ArrayMaxSize, IsArray, IsBoolean, IsIn, IsOptional, IsString, IsUrl, MaxLength, ValidateNested } from 'class-validator';
import {
...
class TradeFlashcardPlaybookConditionDto {
  @ApiProperty({ example: 'range_breakout' })
  @IsString()
  @MaxLength(100)
  playbookType: string;

  @ApiProperty({ example: '动量 K 线强势突破并收盘在区间外' })
  @IsString()
  @MaxLength(1000)
  condition: string;
}
