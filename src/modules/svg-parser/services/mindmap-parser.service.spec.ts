...
describe('MindMapParserService', () => {
  ...
  describe('parseFreeMind', () => {
    it('should parse FreeMind format correctly', async () => {
      const testDataPath = path.join(__dirname, '../test-data/sample-mindmap.mm');
  ...
  describe('parseOPML', () => {
    it('should parse OPML format correctly', async () => {
      const testDataPath = path.join(__dirname, '../test-data/sample-mindmap.opml');
      const content = fs.readFileSync(testDataPath, 'utf-8');
  ...
  describe('parseJSON', () => {
    it('should parse simple-mind-map JSON format correctly', async () => {
      const testDataPath = path.join(__dirname, '../test-data/sample-mindmap.json');
      const content = fs.readFileSync(testDataPath, 'utf-8');
  ...
  describe('parseMarkdown', () => {
    it('should parse Markdown format correctly', async () => {
      const testDataPath = path.join(__dirname, '../test-data/sample-mindmap.md');
      const content = fs.readFileSync(testDataPath, 'utf-8');
  ...
  describe('error handling', () => {
    ...
    it('should throw error for invalid JSON', async () => {
      ...
      await expect(
        service.parseMindMap(invalidJson, 'json')
      ).rejects.toThrow();
    });
  });
});
