...
export function buildClosedPositionsV2(
  ...
  for (const g of groups) {
    ...
    const prevSign = prevQty > 0 ? 1 : prevQty < 0 ? -1 : 0;
    const nextSign = nextQty > 0 ? 1 : nextQty < 0 ? -1 : 0;

    // Handle position reversal (crossing 0) in one-way mode.
    // Important: Binance realizedPnl belongs to the *closing/reducing* portion, not the newly opened portion.
    if (prevSign !== 0 && nextSign !== 0 && prevSign !== nextSign) {
      const closeQty = Math.abs(prevQty);
      const openQty = Math.abs(nextQty);

      // Fee can be roughly split by qty proportion.
      const ratioClose = g.qty > 0 ? closeQty / g.qty : 1;
      const closeFee = g.fee * ratioClose;
      const openFee = g.fee - closeFee;

      // Split fillCount deterministically so sums match.
      const closeFillCount = Math.min(
        g.fillCount,
        Math.max(1, Math.round(g.fillCount * ratioClose)),
      );
      const openFillCount = Math.max(0, g.fillCount - closeFillCount);

      // Closing part
      s.closeSumPxQty += g.vwapPrice * closeQty;
      s.closeSumQty += closeQty;
      s.realizedPnl += g.realizedPnl; // all realized pnl belongs to closing part
      s.fees += closeFee;
      s.feeAsset = s.feeAsset || g.feeAsset;
      s.fillCount += closeFillCount;

      // Force close current session at 0
      const openPrice =
      ...
      // Start a new session with remaining qty (open part)
      const newDirection: 'LONG' | 'SHORT' = nextQty > 0 ? 'LONG' : 'SHORT';
      const ns: Session = {
        symbol,
        ...
        closeTime: g.time,
        openQty: nextQty,
        maxAbsQty: Math.abs(nextQty),
        openSumPxQty: g.vwapPrice * openQty,
        openSumQty: openQty,
        closeSumPxQty: 0,
        closeSumQty: 0,
        realizedPnl: 0, // new opening part has no realized pnl
        fees: openFee,
        feeAsset: g.feeAsset,
        fillCount: openFillCount,
      };
      sessions.set(symbol, ns);
      continue;
    }
