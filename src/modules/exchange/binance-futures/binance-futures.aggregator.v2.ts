...
export function buildClosedPositionsV2(
  ...
  for (const g of groups) {
    ...
    // Handle position reversal within one fill group (crossing 0)
    // Example: prevQty=+0.04, signedQty=-0.06 => nextQty=-0.02
    // We should close the existing LONG session and open a new SHORT session.
    if (prevSign !== 0 && nextSign !== 0 && prevSign !== nextSign) {
      const closeQty = Math.abs(prevQty);
      const openQty = Math.abs(nextQty);

      // Split pnl/fees by qty proportion as a best-effort.
      const ratioClose = g.qty > 0 ? closeQty / g.qty : 1;
      const ratioOpen = g.qty > 0 ? openQty / g.qty : 0;

      // Closing part
      s.closeSumPxQty += g.vwapPrice * closeQty;
      s.closeSumQty += closeQty;
      s.realizedPnl += g.realizedPnl * ratioClose;
      s.fees += g.fee * ratioClose;
      s.feeAsset = s.feeAsset || g.feeAsset;
      s.fillCount += Math.round(g.fillCount * ratioClose);
      ...
      // Start a new session with remaining qty
      ...
      const ns: Session = {
        ...
        closeSumPxQty: 0,
        closeSumQty: 0,
        realizedPnl: g.realizedPnl * ratioOpen,
        fees: g.fee * ratioOpen,
        feeAsset: g.feeAsset,
        fillCount: Math.max(0, g.fillCount - Math.round(g.fillCount * ratioClose)),
      };
